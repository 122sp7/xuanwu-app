...
- When external UI/product specs are provided as links or documents, convert them with markitdown MCP before implementation.
- Capture screenshots for visible UI changes after runtime verification.
- If a UI change alters a reusable pattern, store that pattern in Serena memory so later migration steps stay aligned.
