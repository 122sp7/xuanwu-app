...
interface DispatcherState {
  ...
  /** The task id currently being dragged, for highlight feedback. */
...
export const useDispatcherStore = create<DispatcherState & DispatcherActions>((set) => ({
  ...
  assignTask(taskId, memberId, startTime) {
    set((state) => {
      // Remove any existing assignment for this task before re-assigning
      const filtered = state.assignments.filter((a) => a.taskId !== taskId);
      return {
        assignments: [...filtered, { taskId, memberId, startTime }],
      };
    });
  },
...
export function selectMemberLoad(state: DispatcherState, memberId: string): number {
  const memberAssignments = state.assignments.filter((a) => a.memberId === memberId);
  return memberAssignments.reduce((acc, a) => {
