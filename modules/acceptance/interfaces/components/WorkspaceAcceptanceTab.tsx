...
export function WorkspaceAcceptanceTab({ workspace }: WorkspaceAcceptanceTabProps) {
  const gates = useMemo(() => getWorkspaceAcceptanceGates(workspace), [workspace]);
  const readyCount = useMemo(
    () => gates.filter((gate) => gate.status === "ready").length,
    [gates],
  );
  const overallReady = readyCount === gates.length;
      ...
      <CardContent className="space-y-4">
        <div className="rounded-xl border border-border/40 px-4 py-4">
          <div className="flex flex-wrap items-center gap-2">
            ...
            <Badge variant={overallReady ? "secondary" : "outline"}>
              {overallReady ? "ready-for-acceptance" : "needs-preparation"}
            </Badge>
          </div>
