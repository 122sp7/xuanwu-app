...
function StatusBadge({ doc }: { doc: WikiBetaLiveDocument }) {
...
function RagBadge({ doc }: { doc: WikiBetaLiveDocument }) {
...
/** Upload dropzone + real-time document list backed by Firebase onSnapshot via useDocumentsSnapshot. */
export function WikiBetaDocumentsView({ workspaceId }: WikiBetaDocumentsViewProps) {
  ...
  // Merge pending (optimistic) docs into the real list
  const allDocs = [
    ...pendingDocs,
