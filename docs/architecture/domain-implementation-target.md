...
<!-- change: Add content.page_approved event and process manager plan to v1.1; PR-NUM -->
...
| 功能領域 | 模組 | 備註 |
|---------|------|------|
| Task / Issue 工作流 UI | `workspace-flow` | 核心付費功能，後端已完整，排 v1.1 UI 實作 |
...
| **`ApproveContentPageUseCase`** | `content` | 實作核准 AI 草稿頁面的 Use Case，觸發 `content.page_approved` 事件（見 ADR-001） |
| **`content.page_approved` 完整事件介面** | `content` | 完善事件欄位（actorId、causationId、correlationId），於 `content/domain/events/content.events.ts` |
| **`contentToWorkflowMaterializer` Process Manager** | `workspace-flow` | 監聽 `content.page_approved`，建立帶 `sourceReference` 的 Task / Invoice（見 ADR-001） |
| **`SourceReference` 值物件** | `workspace-flow` | Task / Invoice 聚合根新增 `sourceReference?` 欄位，指回原始 ContentPage |
| **UI 審閱流程（Tasks Tab）** | `workspace-flow` interfaces | 在 Tasks Tab 加入「審閱 → 批准 → 實體化」三步驟 UI（見 `workspace-ui-gap-analysis.md`） |

**ADR 參考：** [`adr/ADR-001-content-to-workflow-boundary.md`](./adr/ADR-001-content-to-workflow-boundary.md)
...
| Notion-style Database | `content/domain/entities/ContentDatabase.ts` | Block DB 視圖，含 `workflow-ref` BlockType 嵌入 Task 狀態 |
