...
| 治理項目 | 歸屬 |
|---|---|
| provider 選擇（Gemini / OpenAI / Vertex AI 等）| `platform.ai` |
| quota 限制、rate limiting | `platform.ai` |
| safety policy、content filtering | `platform.ai` |
| flow 定義（retrieval、synthesis、evaluation）| `notebooklm` |
| prompt template 管理 | `notebooklm`（可委託 `platform.ai` 的 prompt registry）|
...
- 支援多語言、多模型的 prompt 需納入 `platform.ai` 的 prompt registry 治理
...
- ❌ workspace 直接呼叫 platform.ai 或 Genkit（workspace 只能呼叫 notebooklm.api）
- ❌ 不記錄 traceId 的 AI 呼叫
