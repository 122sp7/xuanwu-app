...
- Auth 只在 `platform/subdomains/identity/infrastructure/` 中被 adapter 封裝
- Next.js Server Action 驗證 session 時，呼叫 `platform.api` 的 auth capability，不直接呼叫 `getAuth()`
- `actorId` 由 platform.identity 解析後，以 published language token 傳遞給下游主域
