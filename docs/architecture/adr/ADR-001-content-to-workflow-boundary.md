...
**影響範圍（Scope）：** `modules/knowledge`（Content Buffer Zone），`modules/workspace-flow`, `modules/shared`
...
此外，在探索 `bounded-contexts.md` 的設計後確認：`workspace-flow` 與 `knowledge`（Pages + 計畫中的 Database，在代碼庫中以 `modules/knowledge` 實作）之間存在職責重疊與邊界模糊，需要明確的 DDD 關係模式定義。
...
[AI 攝入管線 py_fn]
      │
      ▼  寫入草稿
[knowledge: KnowledgePage + ContentBlocks]   ← modules/knowledge
      │  使用者審閱、修改
      ▼  核准 (ApproveKnowledgePageUseCase)
[knowledge.page_approved 事件]
      │
      ▼  實體化 (ContentToWorkflowMaterializer)
[workspace-flow: Task / Invoice]
```

`knowledge` 模組（`modules/knowledge`）作為非結構化資料與 AI 解析結果的**緩衝區（Buffer Zone）**，提供 ContentBlock 作為審閱視圖，並在使用者確認後觸發 `knowledge.page_approved` 事件（`KnowledgePageApprovedEvent`）。

### 原則二：事件驅動的實體化（Materialization via Domain Events）

使用者在 `knowledge` 介面確認無誤並「核准（Approve）」後，由 `knowledge` 模組派發領域事件：

```typescript
// modules/knowledge/domain/events/knowledge.events.ts（實際實作）
interface KnowledgePageApprovedEvent {
  readonly type: "knowledge.page_approved";
  readonly aggregateId: string;          // KnowledgePage ID
  readonly occurredAtISO: string;        // ⚠ 注意：目前使用 occurredAtISO（見 ADR-003 異常A）
...
`workspace-flow` 模組由 Process Manager（`ContentToWorkflowMaterializer`，位於 `modules/workspace-flow/application/process-managers/content-to-workflow-materializer.ts`）監聽 `knowledge.page_approved` 事件，進而生成具備狀態機的業務實體。生成的實體**必須**帶有 `sourceReference` 指回原始 `KnowledgePage`：

```typescript
// modules/workspace-flow/domain/value-objects/SourceReference.ts（實際實作）
export type SourceReferenceType = "KnowledgePage";  // ← 非 "ContentPage"

export interface SourceReference {
  readonly type: SourceReferenceType;
  readonly id: string;          // KnowledgePage ID（因果 ID）
  readonly causationId: string;
  readonly correlationId: string;
}
```
...
`knowledge` 模組內的 ContentBlock 可透過 Read Model（CQRS 唯讀查詢）嵌入並展示 `workspace-flow` 的任務最新狀態，但**不允許**直接覆寫其狀態機：

```text
knowledge (ContentPage View) ──reads──► workspace-flow
...
| **上游（Supplier）** | `knowledge`：定義 `knowledge.page_approved` 事件契約（`KnowledgePageApprovedEvent`） |
...
- **解耦 AI 攝入與核心業務**：`py_fn/` 攝入管線只需寫入 `knowledge`，不需了解 `workspace-flow` 的內部結構。
...
| `workspace-flow` 直接讀取 `knowledge` 的 domain 層 | 違反 MDDD 邊界規則（禁止跨模組 domain 層直接 import） |
| `knowledge` 直接呼叫 `workspace-flow` Server Action | 同步耦合，無法保留事件溯源，破壞模組邊界 |
