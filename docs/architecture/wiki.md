...
- **Wiki 頁面 stub**：`app/(shell)/wiki/page.tsx`
  ...
  - 尚無真實持久化，無側邊欄知識節點
- **Knowledge 模組**：`modules/wiki/` (已整合)
  - 目前以 read-side summary 呈現 organization 與 workspace 層級知識狀況
  - Organization 知識：`app/(shell)/wiki/page.tsx` (WikiHubView)
  - Workspace 知識：`modules/workspace/interfaces/components/WorkspaceWikiTab.tsx`
...
## 8. `modules/wiki` 當前實作狀態與交付缺口

> 本節為本輪交付的交底分析。所有標記「✅ 已實作」的項目均在 `modules/wiki/` 中，可供直接引用；
...
| `GetWorkspaceKnowledgeSummaryUseCase` | ✅ 已實作 | 委派 IKnowledgeSummaryRepository，在 modules/wiki 使用 |
...
| `FirestoreKnowledgeSummaryRepository` | 🔲 缺口（高） | 真實資料來源（modules/wiki 使用 DefaultWorkspaceKnowledgeRepository cross-module adapter） |
...
| `modules/knowledge` 模組已整合 | 已完成 | `modules/knowledge` 已全數遷移至 `modules/wiki`；`getWorkspaceKnowledgeSummary` 和 `DefaultWorkspaceKnowledgeRepository` 現居於 `modules/wiki` |
...
└── modules/wiki 已建立並接管 modules/knowledge（已完成）
```
...
| v1.1.0 | 2026-03-20 | 新增第 8 節：modules/wiki 當前實作狀態與交付缺口分析（gap analysis） | xuanwu-app 架構委員會 |
