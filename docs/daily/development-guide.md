...
目前 Daily 已存在的實作，是 **notification-driven digest baseline**：
...
但它**不是最終架構**。未來不要繼續把 Daily 視為 notification list 的另一個外觀。
...
2. **Workspace feed 讀取自己的 DailyProjection**
3. **Organization 聚合所有 workspace feed**
...
3. 讓 facade 逐步改讀 `dailyFeedProjections`
4. 最後再決定是否保留 digest 命名或切換到 feed 命名
