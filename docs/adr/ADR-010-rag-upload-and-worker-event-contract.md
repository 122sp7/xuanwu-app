...
2. `libs/firebase/functions-python/docs/adr/ADR-004-structure-and-interaction-design.md` 與 `ADR-006-enterprise-rag-end-to-end-pipeline.md` 定義 worker 內部流程與觸發方向。
3. 不得將 worker 事件契約改寫成 browser-facing API。
4. 本 ADR 若更新欄位，需同步檢查 functions-python ADR 是否需要補充但不能改變其 worker runtime 定位。
