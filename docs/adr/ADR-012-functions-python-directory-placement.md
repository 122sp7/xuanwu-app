# ADR 012: `functions-python` 目錄位置決策
...
下的對應套件。完成後 `libs/` 目錄僅剩 `libs/firebase/functions-python/` 這一個子目錄，
使得 `libs/` 的存在語意不再成立——它原本是存放 TypeScript 工具整合層的目錄。

此時有必要決定 `functions-python` 的最終放置位置。

## 問題

- `libs/` 目錄原本代表「TypeScript 工具整合層」，遷移後已語意失效
- `functions-python` 不是 TypeScript 套件，無法放入 `packages/`
- 三層路徑 `libs/firebase/functions-python/` 對 Python 工作目錄而言過深且不直觀
- 需要找一個能清楚傳達「這是獨立部署單元」的位置

## 決策 (Decision)

將 `libs/firebase/functions-python/` 移至專案根目錄的 `functions-python/`。

**理由：**

1. **Firebase 慣例**：Firebase CLI 預設 functions codebase 位於專案根目錄（如 `functions/`）；
   `functions-python/` 直接對應 `firebase.json` 中的 codebase 名稱 `functions-python`。

2. **清除語意混淆**：`libs/` 是 TypeScript 工具層，Python worker runtime 不屬於此分類。

3. **第一等部署單元**：放在根目錄明確傳達此目錄是獨立部署單元，而非輔助工具庫。

4. **簡化路徑**：`functions-python/` 比 `libs/firebase/functions-python/` 更短、更易引用。
...
- `functions-python` 內部結構（由各 Python ADR 規範）
...
- `functions-python/` 在根目錄一目了然，與其他 Firebase 設定檔同層
- `firebase.json` source 路徑更短：`"source": "functions-python"`
...
以下檔案已同步更新，將 `libs/firebase/functions-python` 替換為 `functions-python`：
...
- `functions-python/README.md, AGENT.md, docs/adr/*` — 內部自我引用
