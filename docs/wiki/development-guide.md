...
Ingestion 由 `functions-python` 中的 Cloud Functions 處理，與 wiki 模組的介接點：
...
```python
# functions-python/ingestion/taxonomy.py
import openai
...
> Python 端已在 `functions-python/app/rag_ingestion/infrastructure/openai/embedder.py`。
