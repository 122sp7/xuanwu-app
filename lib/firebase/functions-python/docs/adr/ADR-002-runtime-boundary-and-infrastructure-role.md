...
- `lib/firebase/functions`（TypeScript）
- `lib/firebase/functions-python`（Python）
...
4. 遷移 `lib/firebase/functions` 時沒有階段規劃，造成雙邊重複實作與責任重疊。

## 決策 (Decision)

`lib/firebase/functions-python` 被定義為：
