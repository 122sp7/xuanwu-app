...
每個子域 = **`domain/`（名詞：商業狀態實體）** + **`application/`（動詞：計費操作）**。
計費系統子域：`subscription/`（訂閱方案）與 `entitlement/`（有效權益解算）。

## Development Order

```
<subdomain>/domain/ (名詞域：entity / VO / port)
→ <subdomain>/application/ (動詞域：use-case / DTO)
→ adapters/outbound/(stripe|firestore) → adapters/inbound/ → 更新 barrel
```
