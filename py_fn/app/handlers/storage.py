...
流程：
    GCS object.finalized
        → Document AI 直接從 GCS URI 讀取（無記憶體複製）
        → write_parsed_result()

優勢：避免下載大檔案到 Python 函數記憶體。
"""
...
def handle_object_finalized(
    ...
    - Document AI 直接從 GCS 讀取並解析 → 寫入 Firestore。
    """
    data = event.data
    if data is None:
    ...
    logger.info("Processing %s → doc_id=%s", gcs_uri, doc_id)

    # ── 主流程 ──────────────────────────────────────────────────────────────
    # Document AI 直接從 GCS URI 讀取，完全避免下載到記憶體
