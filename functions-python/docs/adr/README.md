...
- 哪些套件可以引入
- 哪些層不可直接相依
- 若要取代 `libs/firebase/functions`，下一步應該先做什麼
...
`libs/firebase/functions` 的既有能力**不能直接硬切移除**。
...
| [ADR-005](./ADR-005-migration-from-typescript-functions.md) | 從 `libs/firebase/functions` 遷移 | 說明共存、替換與下線策略 |
